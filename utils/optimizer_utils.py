...
def get_optimizer(model_obj,loss_type=None,scheduler = False,lr = 0.01):
    ...
    if scheduler == True:
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.01, patience=10, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=1e-7, eps=1e-08)
